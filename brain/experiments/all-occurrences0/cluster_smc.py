...
def load_matrix(npz_path):
    ...
    ci = np.diag(cooccurrence_hist)
    cij = cooccurrence_hist
    T = cooccurrence_n_chunks

    # we need the c00 and c11 values for each feature
    c00 = T - ci[:, np.newaxis] - ci[np.newaxis, :] + cij
    c11 = cij
